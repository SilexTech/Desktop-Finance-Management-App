...
import application.views.payments.ViewCashPayments;
...
public class ControllerCashPayments extends Controller {

    public ControllerCashPayments(Group group) {
        super(new ViewCashPayments(group));
    }

    @Override
    public void init() {
        ViewCashPayments view = (ViewCashPayments) this.view;
        ...
        btnInsert.setOnAction(event -> {
            ...
            if (name == null) return;
            try {
                ModelTransaction mt = new ModelTransaction(null, ModelPerson.getByName(name), amount, date, description, ModelTransaction.Type.CASH_PAYMENT, false, null);
                if(mt.insert()) {
