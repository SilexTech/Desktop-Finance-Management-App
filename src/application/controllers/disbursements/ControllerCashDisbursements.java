...
import application.views.disbursements.ViewCashDisbursements;
...
public class ControllerCashDisbursements extends Controller {
    public ControllerCashDisbursements(Group group) {
        super(new ViewCashDisbursements(group));
    }

    @Override
    public void init() {
        ViewCashDisbursements view = (ViewCashDisbursements) this.view;
        ...
        btnInsert.setOnAction(event -> {
            ...
            if (name == null) return;
            try {
                ModelTransaction mt = new ModelTransaction(null, ModelPerson.getByName(name), amount, date, description, ModelTransaction.Type.CASH_DISBURSEMENT, false, null);
                if(mt.insert()) {
